...
def plot_PPF(A1, B1, A2, B2, user_id, timestamp):
    ...
    if ratio_1 < ratio_2:
        ...
        plt.fill(
            [B1, 0, 0], [A2, A2, y_max], color="lightgray", alpha=0.5
        )  # Треугольник для цеха 1
        plt.fill(
            [B1, B1, x_max], [A2, 0, 0], color="lightblue", alpha=0.5
        )  # Треугольник для цеха 2

    elif ratio_1 > ratio_2:
        plt.plot([0, B2], [y_max, A1], "r--")  # Линия до точки перегиба
        plt.plot([B2, x_max], [A1, 0], "r--")  # Линия до максимума по оси Х
        plt.fill(
            [0, 0, B2], [y_max, A1, A1], color="lightgray", alpha=0.5
        )  # Треугольник для цеха 2
        plt.fill(
            [x_max, B2, B2], [0, 0, A1], color="lightblue", alpha=0.5
        )  # Треугольник для цеха 1
        ...
        plt.fill(
            [0, 0, x_max], [y_max, 0, 0], color="lightgray", alpha=0.5
        )  # Треугольник общий

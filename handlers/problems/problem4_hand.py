...
async def input_q(message: types.Message, state: FSMContext):
    ...
    if await validate_input_int(message, state, "Q"):
        await message.answer(
            "Отлично! Теперь введите цену в штуках (P):", reply_markup=reply.in_task
        )
...
async def input_vc(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if len(variables) > 5:
            ...
            await message.answer(
                "Вы ввели больше 5 строк, в решении будут учтены только первые 5 строк📋",
                reply_markup=reply.in_task,
            )
        for variable in variables[:5]:  # Ограничим ввод до 5 строк
            ...
            # Проверяем, если строка не пустая (если пустая, значит пользователь просто нажал Enter)

...
async def input_p(message: types.Message, state: FSMContext):
    ...
    if await validate_input_int(message, state, "P"):
        await message.answer(
            ...
            " нескольких издержок (вводить одним сообщением, через перенос"
            " строки):\n 'Оплата топлива, 5\n Оплата парковки, 10'\nДля"
            " каждого типа издержек введите от 1 до 5 строк.",
...
async def input_vc(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if text == "нету издержек":
            ...
            await message.answer(
                "Хорошо! Теперь введите данные о постоянных издержках.\nПример"
                " ввода для одной издрежки: 'Аренда зала, 200000'\nПример"
                " ввода для нескольких издержок (вводить одним сообщением,"
        ...
        for variable in variables[:5]:  # Ограничим ввод до 5 строк
            ...
            # Проверяем, если строка не пустая (если пустая, значит просто Enter)
                ...
                await message.answer(
                    ...
                    " нескольких издержок (вводить одним сообщением, через"
                    " перенос строки):\n 'Оплата топлива, 5\n Оплата"
                    " парковки, 10'",
                    reply_markup=reply.costs,
                )
        ...
        await message.answer(
            ...
            " ввода для одной издрежки: 'Аренда зала, 200000'\nПример ввода"
            " для нескольких издержок (вводить одним сообщением, через перенос"
            " строки):\n 'Аренда зала, 200000\n Зарплата"
            " сотрудников, 30000 '",
            reply_markup=reply.costs,
        )
    ...
    try:
            ...
            for variable in variables[:5]:
                ...
                if len(data) == 2:
                    ...
                    await message.answer(
                        ...
                        " для нескольких издержек (вводить одним сообщением,"
                        " через перенос строки):\n 'Аренда зала, 200000\n"
        ...
        if profit >= 0:
            ...
            response += (
                f"\nПри реализации {data['Q']} единиц продукции по"
                f" {data['P']} руб."
                " за единицу товара, убыток составит: "
                f"{abs(profit)} руб."
            )
